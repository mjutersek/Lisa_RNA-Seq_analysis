...
pheno$Biological.group <- gsub("-", "", pheno$Biological.group)
```
...
```{r}
write.table(counts, file="../output/raw_counts.txt", sep="\t", quote=TRUE, row.names=TRUE)
```
...
contrastMatrix = limma::makeContrasts(contrasts = s,levels=design)
```

```{r}
fit2 = contrasts.fit(fit, contrastMatrix)
fit2 <- eBayes(fit2)
plotSA(fit2, main = "SA plot")
```

```{r}
## Merge logFCs and pAdj values
...
for (i in colnames(fit2$coefficients) ) {
  ...
  topTn = cbind(rownames(topTn), topTn)
  colnames(topTn)[1] = 'GeneID'
  
  fn = paste0(sprintf("%03.0f", cnt), '_', i, '.txt')
  
  data.table::fwrite(topTn, file.path(fp, fn), sep = '\t')
...
ind_logFC = grep('GeneID|logFC', colnames(res))
ind_padj = grep('adj.P.Val', colnames(res))
ind = c(ind_logFC, ind_padj)
openxlsx::write.xlsx(res[, ind], file = file.path(fp, fn), asTable = TRUE, overwrite = TRUE)


## Create a table with added raw counts

counts$GeneID = rownames(counts)
merge = merge(res[, ind], counts, by = by, all.x = TRUE, all.y = FALSE)

fn = 'limmaResults_withCounts.xlsx'
openxlsx::write.xlsx(merge, file = file.path(fp, fn), asTable = TRUE, overwrite = TRUE)
```
